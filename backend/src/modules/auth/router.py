...
def _clear_auth_cookies(response: JSONResponse) -> None:
    ...
    if settings.COOKIE_DOMAIN:
        cookie_args["domain"] = settings.COOKIE_DOMAIN
    response.delete_cookie(key=settings.ACCESS_COOKIE_NAME, **cookie_args)
    response.delete_cookie(key=settings.REFRESH_COOKIE_NAME, **cookie_args)
...
async def refresh_session(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if user_id is None:
        raise unauthorized("Invalid refresh session")
